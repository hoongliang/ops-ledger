...
// src/tasks.mjs
var STORE_NAME = "ops-ledger";
var KEY = "todo-tracker-data";
var BACKUP_PREFIX = "backup/";
var BACKUPS_TO_KEEP = 20;
var MAX_BYTES = 1e6;
var BRAND_KEYS = ["hallawaykl", "hallawaypj", "nongfu", "oho", "general"];
function emptyData() {
  const base = { names: [] };
...
function cleanTask(raw) {
  ...
  return {
    ...
    notes: str(raw.notes, 2e3),
    done: raw.done === true,
    completedAt: raw.completedAt ? str(raw.completedAt, 40) : null,
    createdAt: raw.createdAt ? str(raw.createdAt, 40) : (/* @__PURE__ */ new Date()).toISOString()
  };
}
function cleanData(raw) {
  ...
  BRAND_KEYS.forEach((key) => {
    ...
    out[key] = raw[key].slice(0, 2e3).map(cleanTask).filter(Boolean);
  });
...
async function writeBackup(store, record) {
  try {
    await store.setJSON(`${BACKUP_PREFIX}${String(record.rev).padStart(8, "0")}`, record);
    const { blobs } = await store.list({ prefix: BACKUP_PREFIX });
    if (blobs.length > BACKUPS_TO_KEEP + 10) {
      const stale = blobs.map((b) => b.key).sort().slice(0, blobs.length - BACKUPS_TO_KEEP);
      await Promise.all(stale.map((key) => store.delete(key)));
    }
  } catch (err) {
    console.error("Backup skipped:", err);
  }
}
var tasks_default = async (req) => {
  const store = getStore({ name: STORE_NAME, consistency: "strong" });
  if (req.method === "GET") {
    return json(await readRecord(store));
  }
  if (req.method === "PUT") {
    const raw = await req.text();
    if (raw.length > MAX_BYTES) {
      return json({ error: "too_large", message: "That save is over the 1 MB limit." }, 413);
    }
    let body;
    try {
      body = JSON.parse(raw);
